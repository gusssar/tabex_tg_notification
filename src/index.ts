...
type IntervalAndCountPill = { countPill: number; intervalPill: number };
...
const getIntervalAndCountPill = (date: string): IntervalAndCountPill => {
...
const pillIntake = () => {
  ...
  return function (chatId: number, force?: boolean) {
    if (force) {
    ...
    if (count) {
      bot.sendMessage(
        chatId,
        `С этим пока всё, следующая через ${
          interval / 3600000
        } часа, я тебе напомню`
      );
    ...
    bot.sendMessage(
      chatId,
      "На сегодня всё, завтра набери /first_pill_intake чтобы начать приёмный цикл"
    );
  };
};
...
bot.on("message", (msg) => {
  ...
  if (msg.text) {
    ...
    if (/first_pill_intake/.test(text)) {
      ...
      bot.sendMessage(
        chatId,
        "Если это первый приём выбери /first_pill_intake"
      );
    }
  }
});
